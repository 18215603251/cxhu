...
/*封装手机类的分页信息*/
public class DivideCellTypePageService {
	...
	/* 一个页面中分页那部份的数据 */
	private List<CellType> cellTypes;  	// 一个页面包含多条记录
	private int start; 					// 分页的导航中显示的第一个页号
	private int end; 					// 最后一个页号
	
	/*************** 构造页面对象, 包含的是页面中分页部份的信息 ******************/
	public DivideCellTypePageService(int currentPage, int pageRecordSize, int totalSize) {
		...
		/*****分页导航*****/
		if(totalPageSize <= DivideCellTypePageService.nav_max) {   	// 设置最多显示5个分页的导航(可自由设置)
			this.start = 1;
			...
			if (start <= 0) {			 // 左边不够显示的情况
				this.start = 1;
				this.end = DivideCellTypePageService.nav_max;
			}
			if (end >= this.totalPageSize) {			// 右边不够显示的情况
				this.end = totalPageSize;
				this.start = end - DivideCellTypePageService.nav_max + 1;
			}
		}
	}
